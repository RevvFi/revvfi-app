...
const ASSETS = [
  { symbol: "WBTC", name: "Wrapped Bitcoin", address: "0x5FbDB2315678afecb367f032d93F642f64180aa3" },
  { symbol: "WETH", name: "Wrapped Ether", address: "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512" },
  { symbol: "USDC", name: "USD Coin", address: "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0" },
];

const ORACLES = [
  { name: "Pyth Network", desc: "Low latency, cross-chain pricing engine." },
  { name: "Chainlink Data Feeds", desc: "Standard high-integrity decentralized oracle." },
];
...
export default function CreateMarketPage() {
  ...
  const [form, setForm] = useState({
    ...
    collateral_oracle: "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0",
  ...
  async function handleDeploy() {
    await createMarket.mutateAsync({
      ...
      collateral_oracle: form.collateral_oracle,
    ...
    <div className="p-6 max-w-[1200px] mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-on-surface-variant mt-0.5">
          Configure and deploy a custom lending market with enterprise-grade risk parameters and isolated liquidity.
        </p>
      </div>
        ...
        {STEPS.map(({ n, label }, i) => (
          <div key={n} className="flex-1 flex items-center gap-2">
            <div className="flex flex-col items-center gap-1 shrink-0">
              ...
              <p className={cn("text-xs font-medium whitespace-nowrap", step === n ? "text-primary" : "text-on-surface-variant")}>{label}</p>
            </div>
            {i < STEPS.length - 1 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {step === 1 && (
            <Card className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input label="Initial Supply Cap" type="number" value={form.supply_cap}
                  onChange={(e) => setField("supply_cap", e.target.value)} suffix="USDC"
                  hint="Maximum liquidity allowed in this pool." />
                <Input label="Borrow Cap" type="number" value={form.borrow_cap}
                  onChange={(e) => setField("borrow_cap", e.target.value)} suffix="USDC"
                  hint="Limit total debt to maintain health." />
              </div>
            ...
            <Card className="p-6 space-y-5">
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant">Loan-to-Value (LTV)</label>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant">Liquidation Threshold</label>
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant block mb-3">Oracle Infrastructure</label>
            ...
            <Card className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  ["Supply Cap", `$${(parseFloat(form.supply_cap) / 1e6).toFixed(0)}M USDC`],
                  ["Borrow Cap", `$${(parseFloat(form.borrow_cap) / 1e6).toFixed(0)}M USDC`],
                  ["LTV", `${(form.min_collateral_ratio / 100).toFixed(1)}%`],
                  ["Liquidation Threshold", `${(form.liquidation_threshold / 100).toFixed(1)}%`],
              ...
              <div className="rounded-lg border border-amber-400/20 bg-amber-400/5 p-3">
                <p className="text-xs text-amber-400">⚠ Deploying this market will incur a one-time setup fee of 0.25 ETH for gas and protocol validation.</p>
              </div>
            </Card>
          )}
            ...
            <Card className="p-6 flex flex-col items-center gap-5">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-on-surface-variant mt-1">
                  Your market configuration has been verified. Click Deploy to create the on-chain market.
                </p>
              </div>
              <Button size="xl" onClick={handleDeploy} loading={createMarket.isPending} className="gap-2">
                <Rocket className="h-5 w-5" /> Deploy Market
              </Button>
            </Card>
          )}
            ...
            <div className="flex gap-2">
              ...
              {step < 4 ? (
                <Button onClick={() => setStep((s) => (s + 1) as Step)} className="gap-2">
                  {step === 3 ? "Review & Deploy" : "Next"} <ArrowRight className="h-4 w-4" />
                </Button>
              ) : null}
            </div>
          </div>
        ...
        <Card className="p-5 h-fit sticky top-6">
          ...
          <div className="space-y-2.5">
            {[
              ["Supply Cap", `$${(parseFloat(form.supply_cap) / 1e6).toFixed(0)}M`],
              ["Max LTV", `${(form.min_collateral_ratio / 100).toFixed(2)}%`],
              ["Liq. Fee", "1.25%"],
              ["Oracle Feed", ORACLES[form.selectedOracle].name.split(" ")[0] + " V2"],
            ].map(([k, v]) => (
              <div key={k} className="flex justify-between text-sm">
                <span className="text-on-surface-variant">{k}</span>
                <span className={`font-medium ${k === "Liq. Fee" ? "text-emerald-400" : "text-on-surface"}`}>{v}</span>
              </div>
            ))}
          </div>
        </Card>
